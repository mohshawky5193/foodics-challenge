...
- [ ] `Restaurant` entity (`model/entity/Restaurant.java`): `id` (sequence `restaurant_id_seq`), `name`,
      `@OneToMany(mappedBy = "restaurant")` `products`
- [ ] `Supplier` entity (`model/entity/Supplier.java`): `id` (sequence `supplier_id_seq`), `name`,
      `email`, `@OneToMany(mappedBy = "supplier")` `ingredients`
- [ ] `Role` enum (`model/entity/Role.java`): `CUSTOMER`, `RESTAURANT_OWNER`, `SUPPLIER`
- [ ] `User` entity (`model/entity/User.java`): `id` (sequence `user_id_seq`), `email`, `passwordHash`
...
- [ ] `Product` (`model/entity/Product.java`) gains `restaurant` (`@ManyToOne`, column
      `RESTAURANT_ID`, not null) and `price` (`BigDecimal`, column `PRICE`)
- [ ] `Ingredient` (`model/entity/Ingredient.java`) gains `supplier` (`@ManyToOne`, column
      `SUPPLIER_ID`, not null)
- [ ] `RestaurantRepository`, `SupplierRepository`, `UserRepository` (`repository/`), each a plain
      `JpaRepository<T, Long>` — no custom finders needed yet
- [ ] `ProductRepository` (`repository/ProductRepository.java`) gains
...
*Depends on 1.*
- [ ] `003-restaurants-suppliers-accounts-schema.yaml`: `createSequence` for
...
- [ ] `004-seed-restaurant-supplier-accounts.yaml`, context `seed-data`: insert one `restaurant`
...
- [ ] Both new files added to `db.changelog-master.yaml`'s `include` list

## 3. Order flow scoping
*Depends on 1, 2.*
- [ ] `OrderRequest` (`model/request/OrderRequest.java`) gains `Long restaurantId` with `@NotNull`
- [ ] `OrderController.order` (`controller/OrderController.java`) annotates the `@RequestBody` with
      `@Valid` so a missing `restaurantId` produces the existing `MethodArgumentNotValidException` →
      `VALIDATION_FAILED` (`E0002`) path
- [ ] `ProductNotFoundException` (`exception/ProductNotFoundException.java`): constructed with the
      `List<Long>` of missing/foreign product ids, message naming them (e.g.
      `"Product(s) not found: 3, 4"`)
- [ ] `ErrorCode` (`exception/ErrorCode.java`) gains `PRODUCT_NOT_FOUND(HttpStatus.NOT_FOUND)`
      appended after `UNCLASSIFIED_ERROR`, preserving every existing code's number
- [ ] `FoodicsChallengeControllerAdvice` gains a handler for `ProductNotFoundException` →
      `404` with `ErrorCode.PRODUCT_NOT_FOUND`, message from the exception
- [ ] `ProductService.getAllProductsByIdIn` replaced with `getAllProductsByRestaurant(Long
...
- [ ] `OrderService.order` (`service/OrderService.java`) passes `orderRequest.restaurantId()` through
      to `ProductService`

## 4. Alert recipient
*Depends on 1, 2.*
- [ ] `IngredientService` (`service/IngredientService.java`) drops the `MERCHANT_EMAIL` constant;
...
## 5. Tests
*Depends on 1–4.*
- [ ] `OrderControllerTest` (`controller/OrderControllerTest.java`): update existing requests to
...
- [ ] `OrderServiceIntegrationTest` (or wherever the Phase 4/5 integration coverage lives): update
...
- [ ] New test (unit or slice) proving `IngredientService` now emails the ingredient's supplier
      address, not a hard-coded constant
- [ ] New test proving the Liquibase-seeded data matches the exit criteria: one restaurant owning
